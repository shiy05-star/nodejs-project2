const dbConn = require("../../config/dbConfigure");
const inspModel = require("../model/inspModel");

const inspected = async (req, res) => {
    console.log("req.files", req.files);
    const { TIM_Key, inspected_items, timc_status, comments, created_by, TIIM_key, upd_key, uad_key } = req.body;
    if (!TIM_Key || !inspected_items || !timc_status || !comments || !created_by || !TIIM_key || !upd_key || !uad_key) {
        return res.status(400).json({
            success: false,
            error: true,
            message: "All fields are required!"
        });
    }
    console.log(typeof TIM_Key, "TIM_Key");
    if (typeof TIM_Key !== 'string' || typeof inspected_items !== 'string' || typeof timc_status !== 'string' || typeof comments !== 'string' || typeof created_by !== 'string' || typeof TIIM_key !== 'string' || typeof upd_key !== 'string' || typeof uad_key !== 'string') {
        return res.status(400).json({
            success: false,
            error: true,
            message: "Invalid data type for one or more fields"
        });

    }
    // console.log(typeof comments);
    try {
        if (!req.files || req.files.length === 0) {
            return res.status(400).json({
                message: 'Please select an Image.',
            });
        }
        // console.log("hello");

        const result = await inspModel.inspected(req.body);
        console.log(result[0][0].TIMC_KEY, "result");
        console.log(req.files.length, "length")
        const timc_key=result[0][0].TIMC_KEY;
        for (let i = 0; i < req.files.length; i++) {
            console.log(req.files[i].originalname, "asasa");
            console.log("id",timc_key);
            //console.log(req.files[0][i].originalname, "112211asasa");
            const result_images = await inspModel.inspectedImages(timc_key, req.files[i].originalname);
            console.log(result_images[0][0].result, "result");
        }

        return res.status(200).json({
            success: true,
            error:false,
            message: "Details added successfully",

        });
    } catch (err) {

        console.error("Error executing query:", err);
        res.status(500).json({
            success: false,
            error: true,
            message: "Internal server error",
        });
    }
};

module.exports = {
    inspected
};